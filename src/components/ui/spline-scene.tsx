...
function SplineLoader({ className }: { className?: string }) {
  return (
    <div
      className={cn(
        "flex h-full min-h-[480px] w-full items-center justify-center bg-bg-subtle/30",
        className,
      )}
      aria-hidden="true"
    >
...
function SplineCanvas({
  ...
  return (
    <div
      className={cn("relative h-full w-full min-h-[480px]", className)}
      style={{ minHeight: "min(52vh, 560px)" }}
    >
      {phase === "loading" ? (
        <SplineLoader className="absolute inset-0 z-0" />
      ) : null}
      <Spline
        scene={scene}
        onLoad={handleLoad}
...
export function SplineScene({
  ...
  return (
    <ErrorBoundary
      fallback={
        <HeroVisualFallback
          className={cn("h-full min-h-[480px] w-full", props.className)}
          variant={fallbackVariant}
        />
      }
